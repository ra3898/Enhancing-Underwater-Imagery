...
def calculate_psnr_ssim(oppath, data_dir, oppathcsv, resize, width, height):
    ...
    if process.returncode != 0:
        ...
        # st.success("PSNR and SSIM calculation completed successfully!")
        ...
        if st.button("Run Detection and Metrics"):
            if run_detection(name, checkpoint_dir, data_dir, epoch, test_name, result_dir,input_height,input_width):
                ...
                if metrics_file is not None:
                    # st.success("Metrics calculation completed!")
                    ...
                    if os.path.exists(output_path):
                        ...
                        # st.image(output_img, caption="Output Image", use_container_width=True)
                        model = load_model("temp")
                        res = model.infer(output_img,
                                confidence=0.95
                                )
                        detections = sv.Detections.from_inference(res[0].dict(by_alias=True, exclude_none=True))
                        # st.success(len(detections))
                        ...
                        for class_name in class_names:
                                st.text(class_name)
                                if class_name=='shark':
                                    st.error("Shark Ahead, Don't Proceed that Side")
                                else:
                                    st.success("You can Proceed")
                    else:
                        st.warning("Output image not found. Please check.")
                
                    # Display the calculated metrics (CSV file)
                    st.subheader("PSNR and SSIM Metrics")
                    metrics_df = pd.read_csv(metrics_file)
                    st.write(metrics_df, layout="wide")
